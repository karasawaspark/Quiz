...
//  Quiz
//
//  Created by spark-03 on 2023/12/20.
//
...
class QuizViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        self.loadQuiz()
        let panGestureRecongnizer =
        UIPanGestureRecognizer(target: self,
                               action:#selector(dragQuizCard(_:)))
        self.quizCard.addGestureRecognizer(panGestureRecongnizer)
    }
    ...
    func loadQuiz() {
        ...
        //クイズの画面を表示
        self.quizCard.quizImageView.image =
        UIImage(named:  manager.currentQuiz.imageName)
    }
    ...
    func answer() {
        ...
        translationTransform =
            CGAffineTransform(translationX: -screenWidth, y: y)
    ...
    func showNextQuiz() {
        ...
        //transformプロパティに加えられた変更をリセットし、クイズのカードの元の位置に
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        //セグエの遷移先がResultViewController　の場合
